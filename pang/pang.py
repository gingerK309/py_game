...
try:
    ...
    # 배경
    background = pygame.image.load('pang/rsc/background.png')
    textcolor = (0,0,255)
    yellow = (255,255,0)
    pygame.init()
    pygame.font.init()
    ...
    def start():
        ...
        draw_text('Pang game!',80, textcolor,width/2, height*1/5)
        draw_text('shoot: space / move: <-,->',36,  yellow,width/2, height*3/5)
    ...
    def game():
        # 배경음, 효과음
        shoot = pygame.mixer.Sound('pang/rsc/char.wav')
        hit = pygame.mixer.Sound('pang/rsc/hit.wav')
        shoot.set_volume(0.3)
        hit.set_volume(0.3)
        pygame.mixer.music.load('pang/rsc/bgm.ogg')
        pygame.mixer.music.play(-1)

        # 스테이지
        stage = pygame.image.load('pang/rsc/stage.png')
        stage_size = stage.get_rect().size
        stage_height = stage_size[1]

        # 캐릭터
        char = pygame.image.load('pang/rsc/char2.png').convert_alpha()
        ...
        # 무기
        weapon = pygame.image.load('pang/rsc/weapon.png').convert_alpha()
        ...
        # 공
        ball_imgs = [
            pygame.image.load('pang/rsc/balloon1.png').convert_alpha(),
            pygame.image.load('pang/rsc/balloon2.png').convert_alpha(),
            pygame.image.load('pang/rsc/balloon3.png').convert_alpha(),
            pygame.image.load('pang/rsc/balloon4.png').convert_alpha()
        ]
        ...
        while running:
            ...
            if running == False and exitcode == False:
                game_result = 'Game over...'
                draw_text(game_result,56, yellow, width/2, height/2)
            else:
                draw_text(game_result,56, yellow, width/2, height/2)
